...
public class GoogleSheetBusiness {
    ...
    public void updateOldPriceColumn() throws EnptyRowException {
        int endIndexOfBatch = SheetContext.startIndexOfBatch + SheetContext.urls.size() - 1;

        List<List<Object>> oldPrices = getDataInColumnsWithRetry(ApplicationContext.NEW_DATA, ApplicationContext.NEW_DATA, SheetContext.startIndexOfBatch, endIndexOfBatch, ApplicationContext.DATA_SHEET_NAME, 1);
        if (oldPrices == null) {
            throw new EnptyRowException("old price list is empty");
        }
        List<Object> cells = convert2DListToList(oldPrices);

        List<List<Object>> rows = new ArrayList();
        rows.add(cells);

        setDataInColumnsWithRetry(ApplicationContext.OLD_DATA, ApplicationContext.OLD_DATA, SheetContext.startIndexOfBatch, endIndexOfBatch, ApplicationContext.DATA_SHEET_NAME, rows, 1);
    }

    public void keepAllProductDetailColumnInCache(int currentIndex, List<ProductData> productDatas) {
        SellerData sellersData = new SellerData(currentIndex);
    ...
    public void updateAllProductDetailColumns() throws IOException {
        if (SheetContext.sellersData.size() > 0) {
            ...
            setDataInColumnsWithRetry(ApplicationContext.ASIN_CRAWLING, ApplicationContext.WID_3, SheetContext.startIndexOfBatch, SheetContext.currentIdex, ApplicationContext.DATA_SHEET_NAME, rows, 1);
    ...
    public List<List<Object>> getDataInColumnsWithRetry(String columnStart, String columnEnd, int start, int end, String tabName, int second) {
        try {
            return sheetManagement.getDataInColumns(columnStart, columnEnd, start, end, tabName);
            ...
            if (second < 10) {
                ...
                return getDataInColumnsWithRetry(columnStart, columnEnd, start, end, tabName, ++second);
            }
            return null;
        }
    }

    public void setDataInColumnsWithRetry(String columnStart, String columnEnd, int indexStart, int indexEnd, String tabName, List<List<Object>> rows, int second) {
        try {
            sheetManagement.setDataInColumns(columnStart, columnEnd, indexStart, indexEnd, tabName, rows);
        } catch (IOException ioe) {
            logger.error("", ioe);
            if (second < 10) {
                try {
                ...
                setDataInColumnsWithRetry(columnStart, columnEnd, indexStart, indexEnd, tabName, rows, ++second);
            }
        }
    }
}
